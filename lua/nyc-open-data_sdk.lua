...
local NycOpenDataSDK = {}
NycOpenDataSDK.__index = NycOpenDataSDK
...
NycOpenDataSDK._make_feature = _make_feature


function NycOpenDataSDK.new(options)
  local self = setmetatable({}, NycOpenDataSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function NycOpenDataSDK:options_map()
...
function NycOpenDataSDK:get_utility()
  return Utility.copy(self._utility)
end


function NycOpenDataSDK:get_root_ctx()
  return self._rootctx
end


function NycOpenDataSDK:prepare(fetchargs)
...
function NycOpenDataSDK:direct(fetchargs)
...
-- by passing it.
function NycOpenDataSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function NycOpenDataSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "NycOpenDataSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function NycOpenDataSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = NycOpenDataSDK.new(sdkopts)
  sdk.mode = "test"
