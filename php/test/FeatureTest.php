...
class FtCtrl extends NycOpenDataControl
{
...
class FtClient
{
    ...
    private NycOpenDataUtility $utility;

    public function __construct(NycOpenDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): NycOpenDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NycOpenDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NycOpenDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NycOpenDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NycOpenDataConfig::make_feature($name);
    ...
    private static function buildUrl(NycOpenDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NycOpenDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NycOpenDataResponse($fetched);
        }
    ...
    private function fail(NycOpenDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NycOpenDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
