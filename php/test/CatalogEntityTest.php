...
class CatalogEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NYC_OPEN_DATA_TEST_CATALOG_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function catalog_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("NYC_OPEN_DATA_TEST_CATALOG_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["NYC_OPEN_DATA_TEST_CATALOG_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["NYC_OPEN_DATA_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new NycOpenDataSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["NYC_OPEN_DATA_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
