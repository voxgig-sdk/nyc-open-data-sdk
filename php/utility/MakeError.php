...
class NycOpenDataMakeError
{
    public static function call(?NycOpenDataContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new NycOpenDataContext([], null);
        }
        $op = $ctx->op ?? new NycOpenDataOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new NycOpenDataResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof NycOpenDataError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new NycOpenDataError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof NycOpenDataError) {
