...
// NycOpenData SDK Catalog entity
...
class CatalogEntity
{
    ...
    {
        if ($args) {
            $this->_data = NycOpenDataHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = NycOpenDataHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws NycOpenDataError on failure (item-5 convention).
     */
