...
import (
	...
	omni "github.com/voxgig-sdk/nyc-open-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NycOpenDataError); is {
				out := map[string]any{
					"name":    "NycOpenDataError",
...
func omniCtx(arg any, client *sdk.NycOpenDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NycOpenDataSDK); is {
			client = live
