...
class NycOpenDataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NycOpenDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NycOpenDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NycOpenDataTestRunner.getenv("NYC_OPEN_DATA_TEST_LIVE")
        override = NycOpenDataTestRunner.getenv("NYC_OPEN_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NycOpenDataTestRunner.getenv(key)
        ...
        explain = NycOpenDataTestRunner.getenv("NYC_OPEN_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if NycOpenDataTestRunner._test_control is not None:
            return NycOpenDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NycOpenDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NycOpenDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NycOpenDataTestRunner.load_test_control()
...
def load_env_local():
    NycOpenDataTestRunner.load_env_local()


def env_override(m):
    return NycOpenDataTestRunner.env_override(m)


def entity_data(v):
    return NycOpenDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NycOpenDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NycOpenDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NycOpenDataTestRunner.load_test_control()


def live_client_options():
    return NycOpenDataTestRunner.live_client_options()


def live_delay_ms():
    return NycOpenDataTestRunner.live_delay_ms()
