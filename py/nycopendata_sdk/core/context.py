...
from nycopendata_sdk.core.helpers import get_ctx_prop, to_map
...
class NycOpenDataContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, NycOpenDataSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, NycOpenDataResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, NycOpenDataResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return NycOpenDataError(code, msg, self)
