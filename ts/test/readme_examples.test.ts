...
//     (\`new NycOpenDataSDK(...)\` / \`NycOpenDataSDK.test(...)\` become
//     \`NycOpenDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new NycOpenDataSDK(...)\` and any
// \`NycOpenDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NycOpenDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NycOpenDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
