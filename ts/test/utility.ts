...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NYCOPENDATA_TEST_LIVE ||
    'TRUE' === process.env.NYCOPENDATA_TEST_OVERRIDE
  ) {
  ...
  m.NYCOPENDATA_TEST_EXPLAIN = process.env.NYCOPENDATA_TEST_EXPLAIN || m.NYCOPENDATA_TEST_EXPLAIN
