...
[MCP](https://modelcontextprotocol.io) server exposing the NycOpenData SDK as
two agent tools — `nyc-open-data_list` and `nyc-open-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nyc-open-data-mcp)
make build

# 2. Provide credentials via the environment
export NYC_OPEN_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nyc-open-data \
  -- /absolute/path/to/nyc-open-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nyc-open-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nyc-open-data_list: first page of records
{ "entity": "catalog" }
{ "entity": "catalog", "query": { } }

// nyc-open-data_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nyc-open-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nyc-open-data \
     -- "$PWD"/dist/*/nyc-open-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nyc-open-data_list` and `nyc-open-data_load` tools now appear
   in new sessions. Ask the agent to *"list catalog using nyc-open-data"*
   and it calls `nyc-open-data_list` with `{"entity":"catalog"}`.
...
```sh
export NYC_OPEN_DATA_APIKEY=sk_live_xxx            # API key
export NYC_OPEN_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `nyc-open-data_list` tool
...
### Call the `nyc-open-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `nyc-open-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nyc-open-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NYC_OPEN_DATA_APIKEY` | API key sent with every request. |
| `NYC_OPEN_DATA_BASE` | Optional override of the API base URL. |
...
```sh
./nyc-open-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nyc-open-data_load","arguments":{"entity":"catalog","query":{"id":1}}}}'
```
