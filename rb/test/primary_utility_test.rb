...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = NycOpenDataOmni.make_runner(TEST_JSON_FILE, NycOpenDataSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    NycOpenDataError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : NycOpenDataOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = NycOpenDataOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
