...
require_relative 'NycOpenData_types'


class NycOpenDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NycOpenDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NycOpenDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NycOpenDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NycOpenDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NycOpenDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NycOpenDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NycOpenDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => NycOpenDataError.new(
        "#{op}_allow",
        "NycOpenDataSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = NycOpenDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NycOpenDataError.new(
        "graphql_error", "NycOpenDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NycOpenDataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
