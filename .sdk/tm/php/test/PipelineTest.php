...
{
    private static function utility(): NycOpenDataUtility
    {
        return new NycOpenDataUtility();
    }
    ...
    private static function ctx(array $over = []): NycOpenDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new NycOpenDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new NycOpenDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof NycOpenDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new NycOpenDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = NycOpenDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new NycOpenDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = NycOpenDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new NycOpenDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = NycOpenDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NycOpenDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = NycOpenDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new NycOpenDataResponse([]),
            'result' => new NycOpenDataResult([]),
        ]);
        [, $err] = NycOpenDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new NycOpenDataSpec([]),
            'response' => null,
            'result' => new NycOpenDataResult([]),
        ]);
        [, $err] = NycOpenDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new NycOpenDataSpec(['step' => 's']),
            'response' => new NycOpenDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new NycOpenDataResult([]),
        ]);
        [, $err] = NycOpenDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new NycOpenDataSpec(['step' => 's']),
            'response' => new NycOpenDataResponse(self::resp(200, ['v' => 1])),
            'result' => new NycOpenDataResult([]),
        ]);
        [, $err] = NycOpenDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new NycOpenDataSpec(['step' => 's']),
            'response' => new NycOpenDataResponse(self::resp(200, ['v' => 2])),
            'result' => new NycOpenDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        NycOpenDataMakeResponse::call($ctx);
    ...
    {
        $preset = new NycOpenDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new NycOpenDataSpec([]),
            'response' => new NycOpenDataResponse([]),
            'result' => new NycOpenDataResult([]),
        ...
        [$got, $err] = NycOpenDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new NycOpenDataResult([])]);
        [, $err] = NycOpenDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new NycOpenDataSpec([]), 'result' => null]);
        [, $err] = NycOpenDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NycOpenDataSpec(['step' => 's']),
            'result' => new NycOpenDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new NycOpenDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NycOpenDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NycOpenDataSpec(['step' => 's']),
            'result' => new NycOpenDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new NycOpenDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NycOpenDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NycOpenDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new NycOpenDataSpec([]),
            'result' => new NycOpenDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = NycOpenDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NycOpenDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new NycOpenDataError('boom', 'boom');
        $utility->fetcher = function (NycOpenDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NycOpenDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = NycOpenDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (NycOpenDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = NycOpenDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(NycOpenDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        NycOpenDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (NycOpenDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NycOpenDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = NycOpenDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new NycOpenDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new NycOpenDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = NycOpenDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = NycOpenDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = NycOpenDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new NycOpenDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, NycOpenDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new NycOpenDataResult(['ok' => false])]);
        $this->expectException(NycOpenDataError::class);
        NycOpenDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new NycOpenDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', NycOpenDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        NycOpenDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new NycOpenDataBaseFeature();
        $b = new NycOpenDataBaseFeature();
        NycOpenDataFeatureAdd::call($ctx, $a);
        NycOpenDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): NycOpenDataBaseFeature
    {
        $f = new NycOpenDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        NycOpenDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): NycOpenDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new NycOpenDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NycOpenDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        NycOpenDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new NycOpenDataResponse(['headers' => null]),
            'result' => new NycOpenDataResult([]),
        ]);
        NycOpenDataResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        NycOpenDataResultBody::call($ctx);
