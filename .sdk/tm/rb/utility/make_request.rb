...
module NycOpenDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NycOpenDataResponse.new({})
    ...
    if fetch_err
      ...
      response = NycOpenDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NycOpenDataResponse.new(fetched)
    else
