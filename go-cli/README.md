...
boru-driven command-line client **and** interactive REPL for the NycOpenData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nyc-open-data-cli)
make build

# 2. See usage (words, entities, env vars)
./nyc-open-data-cli --help

# 3. Provide credentials once, via the environment
export NYC_OPEN_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./nyc-open-data-cli list catalog

# 5. Override the API base URL for a single call
NYC_OPEN_DATA_BASE=https://api.example.com ./nyc-open-data-cli list catalog

# 6. No arguments -> interactive REPL
./nyc-open-data-cli
nyc-open-data> list catalog
nyc-open-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nyc-open-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nyc-open-data-cli list catalog
   ```
...
Run with no arguments to open a REPL (prompt `nyc-open-data>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nyc-open-data-cli`. |
...
(`./nyc-open-data-cli <expr>`) and interactively (the REPL), and expressions compose the
