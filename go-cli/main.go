// Command NycOpenData-cli is a CLI + REPL for the NycOpenData SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NYC_OPEN_DATA_APIKEY (api key), NYC_OPEN_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NYC_OPEN_DATA_APIKEY carries the API key and
	// NYC_OPEN_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NYC_OPEN_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NYC_OPEN_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNycOpenDataSDK(opts)
